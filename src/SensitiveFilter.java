...
public class SensitiveFilter implements Serializable {
	 ...
	 * 默认的单例，使用自带的敏感词库
	 */
	public static final SensitiveFilter DEFAULT = new SensitiveFilter(new BufferedReader(new InputStreamReader(
			SensitiveFilter.class.getResourceAsStream("sensi_words.txt"), StandardCharsets.UTF_8)));

	/**
	 * 为2的n次方，考虑到敏感词大概在10k左右， 这个数量应为词数的数倍，使得桶很稀疏 提高不命中时hash指向null的概率， 加快访问速度。
	 */
	static final int DEFAULT_INITIAL_CAPACITY = 131072;

	/**
	 * 类似HashMap的桶，比较稀疏。 使用2个字符的hash定位。
	 */
		...
		while (i < sp.length - 2) {
			...
			if (node != null) {
				...
				outer: for (; node != null; node = node.next) {
					/*
					 * 对于一个节点，先根据头2个字符判断是否属于这个节点。 如果属于这个节点，看这个节点的词库是否命中。 此代码块中访问次数已经很少，不是优化重点
					 */
					if (node.headTwoCharMix == mix) {
						/*
						 * 查出比剩余sentence小的最大的词。 例如剩余sentence为"色情电影哪家强？"，
						 * 这个节点含三个词从小到大为："色情"、"色情电影"、"色情信息"。 则从“色情电影”开始向前匹配
						 */
						NavigableSet<StringPointer> desSet = node.words.headSet(sp.substring(i), true);
						if (desSet != null) {
							for (StringPointer word : desSet.descendingSet()) {
								/*
								 * 仍然需要再判断一次，例如"色情信息哪里有？"， 如果节点只包含"色情电影"一个词， 仍然能够取到word为"色情电影"，但是不该匹配。
								 */
